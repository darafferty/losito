...
# -*- coding: utf-8 -*-
"""
Bandpass-operation: Creat soltab for bandpass amplitudelitude.
"""
import os
...
def bandpass(f):
    '''Return the bandpass amplitudelitude for an array of frequencies.'''
    # TODO: Currently, only HBA-low bandpass is included
...
def run(obs, h5parmFilename, stepname='bandpass'): 
    '''
    ...
    bp_amplitude = np.repeat(bp_amplitude[:,np.newaxis], len(source_names), -1)
    weights = np.ones_like(bp_amplitude)
    st = solset.makeSoltab('amplitude', 'amplitude000', axesNames = ['freq','dir'],
